  ...
  return (
    <>
      ...
      <div className="w-full max-w-2xl mx-auto px-4 pt-24 pb-24 lg:pb-8 overflow-x-hidden">
        <h1 className="text-lg font-bold text-slate-900 mb-4">Create Test</h1>
        <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-2xl p-4 space-y-3 w-full">
          ...
          <input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Test title"
            className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm bg-slate-50" />
          ...
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1">Available Until</label>
          ...
          <div>
            ...
            <input type="number" value={form.max_attempts_allowed}
              ...
              className="w-24 px-3 py-2.5 border border-slate-200 rounded-xl text-sm bg-slate-50" />
          </div>
          ...
          <button type="submit" disabled={isLoading}
            ...
            {isLoading && <Loader2 className="w-4 h-4 animate-spin" />} Create Test
          </button>
        </form>
      </div>
    </>
