...
export default function TeacherNotificationsPage() {
  ...
  const load = () => {
    fetch("/api/student/notifications").then((r) => r.json()).then((d) => setItems(d.results || [])).finally(() => setIsLoading(false));
  };
  useEffect(() => { load(); }, []);

  const markRead = async (id: string) => { await fetch(`/api/student/notifications/${id}/read`, { method: "POST" }); load(); };
  const markAll = async () => { await fetch("/api/student/notifications/mark-all-read", { method: "POST" }); load(); };
