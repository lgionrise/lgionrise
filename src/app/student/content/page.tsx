  ...
  useEffect(() => {
    ...
    fetch(`/api/student/content${params}`)
      .then(async (r) => {
        ...
        if (!r.ok) {
          setError(d.error || "Could not load study material.");
          return;
        }
        setItems(d.results || []);
      })
  ...
  return (
    <>
      ...
      <div className="w-full max-w-2xl mx-auto px-4 pt-24 pb-24 lg:pb-8 overflow-x-hidden">
        ...
        {error && (
          <p className="text-sm text-red-600 bg-red-50 rounded-xl px-3.5 py-2.5 mb-4 break-words">{error}</p>
        )}
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {items.map((item) => (
              <a key={item.public_id} href={item.file_url} target="_blank" rel="noopener noreferrer" className="bg-white rounded-2xl p-3.5 shadow-sm flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-500">
                    ...
                    {item.batch_title ? ` • ${item.batch_title}` : ""}
                  </p>
                </div>
