...
export default function TeacherProfilePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (!meResult.ok) { setError(`Session error (${meResult.status}).`); return; }
      if (!profileResult.ok) { setError(`Profile load failed (${profileResult.status}).`); return; }
  ...
  if (isLoading) return <p className="text-slate-500 text-sm p-5 pt-24">Loading...</p>;
  if (error) return <p className="text-red-600 text-sm p-5 pt-24 bg-red-50 rounded-xl m-5">{error}</p>;
  if (!profile || !account) return <p className="text-red-600 text-sm p-5 pt-24">Could not load profile.</p>;
  ...
  return (
    <div className="w-full max-w-xl mx-auto px-4 pt-24 pb-24 lg:pb-8 overflow-x-hidden">
      <div className="bg-gradient-to-br from-indigo-600 to-violet-600 rounded-2xl p-5 text-center text-white mb-5 w-full">
        ...
        <p className="text-white/70 text-xs break-all px-2">{account.email}</p>
      ...
      <form onSubmit={handleSave} className="bg-white border border-slate-200 rounded-2xl p-4 space-y-4 w-full">
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1.5">Bio</label>
          <textarea rows={3} value={profile.bio} onChange={(e) => setProfile({ ...profile, bio: e.target.value })}
            className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm bg-slate-50 resize-none" />
        </div>
        <div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {profile.subjects.map((subject, i) => (
              <span key={i} className="flex items-center gap-1.5 bg-indigo-50 text-indigo-700 text-xs font-medium px-2.5 py-1 rounded-full max-w-full">
                <span className="truncate max-w-[120px]">{subject}</span>
                <button type="button" onClick={() => removeSubject(i)} className="shrink-0"><X className="w-3 h-3" /></button>
              </span>
            ))}
          </div>
          <div className="flex gap-2 w-full">
            <input
              ...
              placeholder="Add a subject" className="flex-1 min-w-0 px-3 py-2 border border-slate-200 rounded-xl text-sm bg-slate-50"
            />
            <button type="button" onClick={addSubject} className="shrink-0 bg-slate-100 text-slate-700 text-sm font-medium px-4 rounded-xl">Add</button>
          </div>
        </div>
        ...
        <label className="flex items-start gap-2.5 text-sm text-slate-700">
          <input type="checkbox" checked={profile.is_profile_visible}
            ...
            className="rounded border-slate-300 text-indigo-600 mt-0.5 shrink-0" />
          <span>Make profile visible to students</span>
        </label>

        {error && <p className="text-sm text-red-600 bg-red-50 border border-red-100 rounded-xl px-3.5 py-2.5 break-words">{error}</p>}
