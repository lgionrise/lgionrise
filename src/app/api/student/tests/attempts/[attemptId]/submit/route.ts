// src/app/api/student/tests/attempts/[attemptId]/submit/route.ts
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ attemptId: string }> }) {
  ...
  const res = await djangoFetch(`/tests/attempts/${attemptId}/submit/`, { method: "POST", body: JSON.stringify(body) });
