// src/app/api/teacher/live-classes/[publicId]/cancel/route.ts
...
export async function POST(request: NextRequest, { params }: { params: Promise < { publicId: string } > }) {
  ...
  const res = await djangoFetch(`/live-classes/${publicId}/cancel/`, { method: "POST", body: JSON.stringify(body) });
